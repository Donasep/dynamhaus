...
use Exception;
require dirname(__DIR__,3).'/config/dbconfig.php';
abstract class AbstractDataManager {
private ?\PDO $db=null;
    public function connect(): \PDO {
        if ($this->db===null) {
            $host = DBCONFIG['data']['host'];
$db   = DBCONFIG['data']['dbname'];
$user = DBCONFIG['data']['username'];
$pass = DBCONFIG['data']['password'];
$port = 3306;

// Chemin vers le certificat CA (à télécharger ici : https://www.digicert.com/CACerts/BaltimoreCyberTrustRoot.crt.pem)
$ca_cert = DBCONFIG['data']['ca_cert'];
...
try {
    ...
    protected function updateSensorParam(string $class, array $fields, int $sensorId): \PDOStatement {
		$query = "UPDATE " . $this->getTableName($class) . " SET ";
		foreach (array_keys($fields) as $field) {
			$query .= $field . " = :" . $field;
			if ($field != array_key_last($fields)) $query .= ', ';
		}
		$query .= ' WHERE sensorId = :sensorId';
		$fields['sensorId'] = $sensorId;
		return $this->executeQuery($query, $fields);
	}
    ...
    protected function readOneWithParam(string $class, array $filters = [], array $order = [], int $limit = null, int $offset = null) {
        $query = 'SELECT * FROM ' . $this->getTableName($class) . ' WHERE ';
        foreach (array_keys($filters) as $filter) {
            $query .= $filter . " = :" . $filter;
            if ($filter != array_key_last($filters)) $query .= ' AND ';
